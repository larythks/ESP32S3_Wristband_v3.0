...
| 功能模块 | 具体内容 |
|---------|---------|
| 体温监测 | DS18B20 采集、阈值报警（≥37.8°C / ≤35.0°C） |
...
| 项目 | 内容 |
|-----|------|
| 职责 | 传感器采样调度（常规/实时双模式）、滑动平均滤波、数据融合 |
...
3. 实现体温滑动平均 + 连续触发判定（连续 3 次有效样本）
4. 实现单点报警阈值判定（无区间预警）与 ALARM 事件发布
5. 实现计步算法（加速度峰值检测）
6. UI 显示心率、血氧、步数，并显示“测量无效”状态
7. 首次越阈时，触发对应传感器进入实时检测窗口并执行二次确认
