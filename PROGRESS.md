...
### 2026-02-16 - Bug 修复: 手机配对后立即断开连接 (ISSUE-011)

- **迭代**: Bug 修复（影响迭代 2.5 BLE 连接稳定性）
- **状态**: ✅ 已完成
- **问题描述**: 手机与 ESP32 配对成功后（加密建立、绑定完成），连接立即断开
- **根本原因**: `CONFIG_BT_NIMBLE_GATT_CACHING=y` 导致重复配对时 GATT 缓存状态不一致，手机收到 Service Changed 后断开连接
- **修改文件**:
  - sdkconfig（关闭 `CONFIG_BT_NIMBLE_GATT_CACHING`）
  - components/ble_gatt/ble_service.c（增强 Subscribe 和 Disconnect 诊断日志）
- **验收状态**: 待验收
- **验收清单**:
  - [ ] 手机配对后连接不再自动断开
  - [ ] 断连时日志显示 reason 的十进制和十六进制值
  - [ ] Subscribe 日志显示 cur_notify 和 cur_indicate 两个字段
- **备注**:
  - 关闭 GATT 缓存对功能无影响，仅每次重连时手机需做完整服务发现（延迟可忽略）
  - 如问题仍存在，需根据断连 reason 码进一步排查

---

### 2026-02-16 - Bug 修复: BLE 连接后不触发配对 (ISSUE-012)

- **迭代**: Bug 修复（影响迭代 2.5 BLE 安全功能）
- **状态**: ✅ 已完成
- **问题描述**: 手机连接后不弹出配对请求，无需配对即可订阅通知
- **根本原因**: `ble_gap_security_initiate()` 被注释掉，ESP32 不主动发起安全请求
- **修改文件**:
  - components/ble_gatt/ble_service.c（取消注释 `ble_gap_security_initiate()`）
- **验收状态**: 待验收
- **验收清单**:
  - [ ] 手机连接后弹出配对请求
  - [ ] 配对成功后连接保持稳定
  - [ ] 日志显示 Encryption change 和 Security 状态
